...
namespace QLNS1.Controllers
{
    public class ImportBooksController : Controller
    {
        ...
        public ImportBooksController(QLNS1Context context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Create([Bind("Id,TenSach,TacGia,TheLoai,AmountImport,DateImport,SachSauNhap")] NhapSach nhapSach)
        {
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    if (sach == null)
                    {
                        ModelState.AddModelError("TenSach", "Sách không tồn tại");
                        return View(nhapSach);
                    }
                    else
                    {
                        if (sach.Amount >= 200)
                        {
                            ModelState.AddModelError("AmountImport", "Số lượng tồn hơn 200");
                            return View(nhapSach);
                        }
